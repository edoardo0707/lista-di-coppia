...
function toggleContainer(id) {
  ...
  // Mostra solo se non era già visibile
  if (!isVisible) {
...
// --- Carica le città da Firestore all’avvio ---
...
function saveCityToFirestore(city) {
  db.collection('cities').add(city)
    ...
    .catch(err => {
      console.error('Errore salvataggio città:', err);
    });
}

const globe = Globe()
  .globeImageUrl('jpeg-optimizer_jpeg-optimizer_world.topo.bathy.200412.3x21600x10800.jpg')
  ...
  .pointAltitude(0.02)
  .pointRadius(0.4)
  .pointOfView({ lat: 0, lng: 0, altitude: 5 }, 1000)
  .onPointClick(point => {
...
// Funzione geocoding (la tua, immutata)
...
input2.addEventListener('keydown', async e => {
  if (e.key === 'Enter' && input2.value.trim() !== '') {
    ...
    if (city) {
      // Controllo doppioni (case insensitive)
      if (!cities.some(c => c.name.toLowerCase() === city.name.toLowerCase())) {
        cities.push(city);
        globe.pointsData(cities);
        updateCityList();
        globe.pointOfView({ lat: city.lat, lng: city.lng, altitude: 0.7 }, 1000);
        saveCityToFirestore(city); // SALVA su Firestore
      } else {
...
// Carica le città da Firestore all’avvio della pagina
